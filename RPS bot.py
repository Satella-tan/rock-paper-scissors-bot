    ...
    while valid_rounds < number_of_rounds:
        ...
        # every two moves bot plays based on eu
            ...
            if random.random() < epsilon:
                ...
                # 1. Use the empirical probabilities we already calculated
                probs = empirical_prob_moves
                
                # 2. Entropy already calculated above
                
                # 3. Map entropy → inverse temperature beta
                beta = beta_max * (Hmax - entropy) / Hmax
                print(f"Beta (inverse temperature): {beta:.3f}")
                
                # 4. Compute expected utility for each bot move
                # EU = chance that each bot move wins against user distribution
                EU = [
                    probs[2] - probs[1],  # Rock beats Scissors, loses to Paper
                    probs[0] - probs[2],  # Paper beats Rock, loses to Scissors
                    probs[1] - probs[0]   # Scissors beats Paper, loses to Rock
                ]
                print(f"EU for [R, P, S]: {EU}")
                
                # 5. Softmax to get adaptive action probabilities
                maxEU = max(EU)
                ...
                # 6. Sample a move using those probabilities
                r = random.random()
                cumulative = 0
